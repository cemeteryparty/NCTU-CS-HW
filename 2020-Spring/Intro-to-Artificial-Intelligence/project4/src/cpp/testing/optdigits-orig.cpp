...
int Predict(Node* node, bool picture[1024]);
void PrintTree(Node* node);
void BuildDecisionTree(Node* node);
Children Partition(vector<Sample> & dataset, int & attribute, bool threshold);
...
int main(int argc, char const *argv[]){
	...
	ReadDataset("optdigits-orig.tra",digits_train);
	...
	ReadDataset("optdigits-orig.cv",digits_test);
	int hit = 0;
...
void BuildDecisionTree(Node* node){
	...
	for(int attribute = 0;attribute < 1024;attribute++){
		// calculate possible threshold
		vector<bool> thres_arr;
		thres_arr.push_back(true);
		thres_arr.push_back(false);
...
void PrintTree(Node* node){
	...
	while(!q.empty()){
		Node *n = q.front();
		q.pop();
		printf("index, attr, thres, val = %d, (%d,%d), %d, %d\n",n->index,n->attr / 32,n->attr % 32,n->threshold,n->leaf);
		if(n->branch_t)
			q.push(n->branch_t);
		if(n->branch_f)
			q.push(n->branch_f);
