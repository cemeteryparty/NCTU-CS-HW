...
public:
    ...
    Variable(int val = -1){
        value = val; 
        position = -1;
        mdl = mdu = INT_MAX; // MIN_DISTANCE_TO_LOWER, MIN_DISTANCE_TO_UPPER
...
void print_solution(vector<Variable> & solution);
void BruteForce(vector<Variable> & board,vector<Constrain> & cons);
void Optimal_MRVDegree(vector<Variable> & board,vector<Constrain> & cons);
void Optimal_MRVDegreeLCV(vector<Variable> & board,vector<Constrain> & cons);
int n,m,NumofMines;
int main(int argc, char const *argv[]){
    ...
    Optimal_MRVDegree(board,cons);
    printf("Case Optimal(MRV+Degree+LCV):\n");
    Optimal_MRVDegreeLCV(board,cons);
    return 0;
}
void BruteForce(vector<Variable> & board,vector<Constrain> & cons){
    ...
    while(!stack.empty()){
        ...
        for(int i = 0;i < cons.size();i++){ 
            ...
            if(lower == cons[i].value && upper == cons[i].value)
                ...
                for(int j = 0;j < cons[i].variable.size();j++){
                    if(node[cons[i].variable[j]].mdu > upper - cons[i].value) 
                        node[cons[i].variable[j]].mdu = upper - cons[i].value;
                    if(node[cons[i].variable[j]].mdl > cons[i].value - lower) 
                        node[cons[i].variable[j]].mdl = cons[i].value - lower;
                }
                goal = false; // NOT goal node
            }
        }
